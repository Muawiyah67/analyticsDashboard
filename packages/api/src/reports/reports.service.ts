...
export class ReportsService {
  ...
  async findAll(companyId: string, page = 1, limit = 10, type?: string, status?: string) {
  ...
  async getPdfUrl(slug: string): Promise<string> {
    const report = await this.findBySlug(slug);
    
    if (!report.file_url) {
      throw new NotFoundException('PDF not found for this report');
    }
  ...
  private async generatePdfContent(companyId: string, type: string, title: string, csvContent: string): Promise<Buffer> {
  ...
  private async generateCsvContent(companyId: string, type: string): Promise<string> {
    let data: any[] = [];
    let columns: string[] = [];
